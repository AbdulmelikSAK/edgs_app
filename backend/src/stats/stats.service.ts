...
export class StatsService {
  ...
  async getGlobalStats(from?: string, to?: string) {
    ...
    const totalRevenue = completed.reduce(
      (sum, m) => sum + (Number(m.actualPrice) || 0),
      0,
    );
    const totalEstimated = completed.reduce(
      (sum, m) => sum + (Number(m.estimatedPrice) || 0),
      0,
    );
    const totalSurface = completed.reduce(
      (sum, m) => sum + (Number(m.surfaceArea) || 0),
      0,
    );
    const totalFuel = completed.reduce(
      (sum, m) => sum + (Number(m.fuelConsumption) || 0),
      0,
    );
    const totalSandBags = completed.reduce(
      (sum, m) => sum + (m.sandBagsUsed || 0),
      0,
    );
    ...
    return {
      missions: {
        ...
        planned: missions.filter((m) => m.status === MissionStatus.PLANNED)
          .length,
        cancelled: missions.filter((m) => m.status === MissionStatus.CANCELLED)
          .length,
      },
      financial: {
        totalRevenue,
  ...
  async getTruckStats(truckId: string) {
    ...
    return {
      ...
      completed: missions.filter((m) => m.status === MissionStatus.COMPLETED)
        .length,
      totalRevenue: missions.reduce(
        (s, m) => s + (Number(m.actualPrice) || 0),
        0,
      ),
      totalSurface: missions.reduce(
        (s, m) => s + (Number(m.surfaceArea) || 0),
        0,
      ),
  ...
  async getDashboardStats(): Promise<any> {
    ...
    employees.forEach((emp) => {
      ...
      if (clockedInTime) {
        const limitTime =
          new Date().getTime() < endOfToday.getTime() ? new Date() : endOfToday;
      ...
      if (pauseStartTime) {
        const limitTime =
          new Date().getTime() < endOfToday.getTime() ? new Date() : endOfToday;
        totalPauseMs +=
          limitTime.getTime() - (pauseStartTime as Date).getTime();
      }
    ...
    const m2RealisesToday = productionToday.reduce(
    ...
    const avancementChantiers = await Promise.all(
      allMissions.map(async (m) => {
        ...
        const totalRealised = prod.reduce(
          (sum, p) => sum + Number(p.quantity),
          0,
        );
        ...
        const percentage = Math.min(
          100,
          Math.round((totalRealised / totalPlanned) * 100),
        );
    ...
    const rentabilitéChantiers = await Promise.all(
      allMissions.map(async (m) => {
        ...
        const displacementCost =
          timeEntries.filter((t) => t.displacementMode === 'grand_deplacement')
            .length *
            120 +
        ...
        const coutReel =
          mainDOeuvreCost + displacementCost + sandCost + fuelCost;

        // Marges
        const margeBrute = caRealise - coutReel;
        const tauxMarge =
          caRealise > 0 ? Math.round((margeBrute / caRealise) * 1000) / 10 : 0;
